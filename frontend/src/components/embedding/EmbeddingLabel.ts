...
export function drawLabels(
  ...
  const enterFunc = (
    ...
    const text = labelGroup
      ...
      .style('stroke', '#fff')
      .style('stroke-width', 1.8 / this.curZoomTransform.k);
...
export function mouseoverLabel(
  ...
  // No topic data (e.g. simplified grid) -> nothing to highlight.
  if (this.topicLevelTrees.size === 0) return;
...
export function redrawTopicGrid(
  ...
  for (const topicCtx of topicCtxs) {
    ...
    topicCtx.clearRect(0, 0, this.svgFullSize.width * dpr, this.svgFullSize.height * dpr);
    topicCtx.scale(dpr, dpr);
