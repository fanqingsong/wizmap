...
export class Embedding {
    ...
    for (const pos of ['top', 'bottom']) {
      this.topicCanvases.push(
        d3
          ...
          .attr('width', `${this.svgFullSize.width * dpr}px`)
          .attr('height', `${this.svgFullSize.height * dpr}px`)
          .style('width', `${this.svgFullSize.width}px`)
          .style('height', `${this.svgFullSize.height}px`)
  ...
  initData = async () => {
    ...
    // Create a quad tree at each level. The simplified grid generator does not
    // produce topic data, so skip gracefully when it is absent (the topic
    // label/grid functions all no-op when topicLevelTrees is empty).
    if (this.gridData.topic?.data) {
